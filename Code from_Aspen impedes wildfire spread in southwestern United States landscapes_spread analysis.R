...
#Load data:
setwd("C:/Users/matth/Desktop/Thesis Dev/Final Data and Code")
...
#Make percent aspen per fire dataframe to make 0.1% aspen per fire subset (used in final analysis)
#First make daily area of aspen
DAA<-Aspen_DOB[,c(1,3,4,5,7)]
DAA$DAA<-DAA$GC_Percent*DAA$DOB_Area_m2
SDAA_Sum <- DAA %>%
  group_by(Event_ID) %>%
  summarize(SDAA = sum(DAA, na.rm = TRUE))
joined_DAA <- inner_join(DAA, SDAA_Sum, by = "Event_ID")
#Second make sum area by fire event
SAFE<-Aspen_DOB[,c(1,4,5)]
SAFE_Sum <- SAFE %>%
  group_by(Event_ID) %>%
  summarize(Fire_area = sum(DOB_Area_m2, na.rm = TRUE)) 
#Third make % aspen per fire 
joined_df <- inner_join(joined_DAA, SAFE_Sum, by = "Event_ID")

joined_df$PAPF<-joined_df$SDAA/joined_df$Fire_area

#Create percent aspen per fire subsets: 
Aspen_0.1P<- subset(joined_df, PAPF >= 0.1)
Aspen_1P<- subset(joined_df, PAPF >= 1)
Aspen_5P<- subset(joined_df, PAPF >= 5)
Aspen_10P<- subset(joined_df, PAPF >= 10)
Aspen_25P<- subset(joined_df, PAPF >= 25)

#Now inspect models for different %aspen per fire subsets: 
#Base model:
model1<-glmmTMB(log10(DOB_Area_m2)~GC_Percent +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(model1)#-0.004577

#Aspen_0.1P model******
model2<-glmmTMB(log10(DOB_Area_m2)~GC_Percent +(1|Event_ID), family=gaussian, data=Aspen_0.1P)
summary(model2)
...
R2
#Aspen_1P model
model3<-glmmTMB(log10(DOB_Area_m2)~GC_Percent +(1|Event_ID), family=gaussian, data=Aspen_1P)
summary(model3)

#Aspen_5P model******
model4<-glmmTMB(log10(DOB_Area_m2)~GC_Percent +(1|Event_ID), family=gaussian, data=Aspen_5P)
summary(model4)

#Aspen_10P model******
model5<-glmmTMB(log10(DOB_Area_m2)~GC_Percent +(1|Event_ID), family=gaussian, data=Aspen_10P)
summary(model5)

#Aspen_25P model
model6<-glmmTMB(log10(DOB_Area_m2)~GC_Percent +(1|Event_ID), family=gaussian, data=Aspen_25P)
summary(model6)

###Distance Spread 
Aspen_DOB$DOB_Max_DS
...
#Aspen_0.1P model******
model2<-glmmTMB(log10(DOB_Max_DS)~GC_Percent +(1|Event_ID), family=gaussian, data=Aspen_0.1P)
summary(model2)
...
R2
#Aspen_1P model
model3<-glmmTMB(log10(DOB_Max_DS)~GC_Percent +(1|Event_ID), family=gaussian, data=Aspen_1P)
summary(model3)

#Aspen_5P model******
model4<-glmmTMB(log10(DOB_Max_DS)~GC_Percent +(1|Event_ID), family=gaussian, data=Aspen_5P)
summary(model4)

#Aspen_10P model******
model5<-glmmTMB(log10(DOB_Max_DS)~GC_Percent +(1|Event_ID), family=gaussian, data=Aspen_10P)
summary(model5)

#Aspen_25P model
model6<-glmmTMB(log10(DOB_Max_DS)~GC_Percent +(1|Event_ID), family=gaussian, data=Aspen_25P)
summary(model6)

#Now make figures based off of the selected 0.1%, 5%, and 10% aspen models 
...
eff_data <- as.data.frame(Effect(mod = model2,
                                 term = "GC_Percent",
...
E5 <- as.data.frame(Effect(mod = model4,
                           term = "GC_Percent",
                           focal.predictors = "GC_Percent",
                           xlevels = 100))
# Back-transform DOB_Area
E5$fit <- 10^E5$fit/10000
E5$lower <- 10^E5$lower/10000
E5$upper <- 10^E5$upper/10000

E10 <- as.data.frame(Effect(mod = model5,
                            term = "GC_Percent",
                            focal.predictors = "GC_Percent",
                            xlevels = 100))
# Back-transform DOB_Area
...
# Daily Area Burned 
DABW <- ggplot() +
  geom_line(data = E5, aes(x = GC_Percent, y = fit, color = ">5% Aspen"), linewidth = 1.2, lineend = "round", linetype = "longdash") +
  geom_ribbon(data = E5, aes(x = GC_Percent, ymin = lower, ymax = upper, fill = ">5% Aspen 95% CI"), alpha = 0.2) +
  geom_line(data = E10, aes(x = GC_Percent, y = fit, color = ">10% Aspen"), linewidth = 1.2, lineend = "round", linetype = "dotted") +
  ...
  scale_color_manual(values = c("Aspen Present" = "#00A6B6", ">5% Aspen" = "#7B02A8", ">10% Aspen" = "#F1731D")) +
  scale_fill_manual(values = c("Aspen Present 95% CI" = "#00A6B6", ">5% Aspen 95% CI" = "#7B02A8", ">10% Aspen 95% CI" = "#F1731D")) +
  theme_classic() +
...
# Back-transform DOB_Area
E5$fit <- 10^E5$fit
E5$lower <- 10^E5$lower
E5$upper <- 10^E5$upper
...
DLSW <- ggplot() +
  ...
  labs(x = "Aspen Cover (%)",
       ...
       fill = "") +
  scale_x_continuous(limits = c(0, 100)) +
  scale_color_manual(values = rev(c("Aspen Present" = "#00A6B6", ">5% Aspen" = "#7B02A8", ">10% Aspen" = "#F1731D"))) +
  scale_fill_manual(values = rev(c("Aspen Present 95% CI" = "#00A6B6", ">5% Aspen 95% CI" = "#7B02A8", ">10% Aspen 95% CI" = "#F1731D"))) +
  theme_classic() +
  theme(axis.text.x = element_text(hjust = 0.5), 
        axis.text.y = element_text(hjust = 0.5),
        legend.position = c(0.95, 0.95),  # Position the legend in the top right
...
ggsave("C:/Users/matth/Desktop/Thesis Dev/Final Figures/ROS.multipanel.tiff",
       plot = ROS.multipanel,
       width = 16,
       height = 8,
       units = "in",
...
#Now create percent aspen per DOB subsets using Aspen_0.1P for 0.1% aspen per fire event selection threshold: 
###Burned Area 
#0-10% aspen
Aspen_L10PDOB<- subset(Aspen_0.1P, GC_Percent < 10)
mean(Aspen_L10PDOB$DOB_Area_m2)/10000

#Stdev:
std_dev<-sd(Aspen_L10PDOB$DOB_Area_m2)/10000
n<-length(Aspen_L10PDOB$DOB_Area_m2)
sem <- std_dev / sqrt(n)
sem

#10-25% aspen
#10-25
Aspen_M10PDOB<- subset(Aspen_0.1P, GC_Percent > 10)
Aspen_10_25PDOB<- subset(Aspen_M10PDOB, GC_Percent < 25)
mean(Aspen_10_25PDOB$DOB_Area_m2)/10000

#Stdev:
std_dev<-sd(Aspen_10_25PDOB$DOB_Area_m2)/10000
n<-length(Aspen_10_25PDOB$DOB_Area_m2)
sem <- std_dev / sqrt(n)
sem

#>25% aspen
#25 
Aspen_L25PDOB<- subset(Aspen_0.1P, GC_Percent > 25)
mean(Aspen_L25PDOB$DOB_Area_m2)/10000

#Stdev:
std_dev<-sd(Aspen_L25PDOB$DOB_Area_m2)/10000
n<-length(Aspen_L25PDOB$DOB_Area_m2)
sem <- std_dev / sqrt(n)
sem
...
#0-10% aspen
Aspen_L10PDOB<- subset(Aspen_0.1P, GC_Percent < 10)
mean(Aspen_L10PDOB$DOB_Max_DS)

#Stdev:
std_dev<-sd(Aspen_L10PDOB$DOB_Max_DS)
n<-length(Aspen_L10PDOB$DOB_Max_DS)
sem <- std_dev / sqrt(n)
sem
...
Aspen_M10PDOB<- subset(Aspen_0.1P, GC_Percent > 10)
Aspen_10_25PDOB<- subset(Aspen_M10PDOB, GC_Percent < 25)
mean(Aspen_10_25PDOB$DOB_Max_DS)

#Stdev:
std_dev<-sd(Aspen_10_25PDOB$DOB_Max_DS)
n<-length(Aspen_10_25PDOB$DOB_Max_DS)
sem <- std_dev / sqrt(n)
sem
...
#25 
Aspen_L25PDOB<- subset(Aspen_0.1P, GC_Percent > 25)
mean(Aspen_L25PDOB$DOB_Max_DS)

#Stdev:
std_dev<-sd(Aspen_L25PDOB$DOB_Max_DS)
n<-length(Aspen_L25PDOB$DOB_Max_DS)
sem <- std_dev / sqrt(n)
sem

###Now run model selection on all covariates and define best fit models
#Load data:
setwd("C:/Users/matth/Desktop/Thesis Dev/Final Data and Code")
ALL_LC_df<-read.csv("Harris_Aspen_Fire_Spread_Data.csv")#Formerly "GPLXCovariate_df_01P.csv"

#Match 0.1% Aspen selection: 
Fire_Selection<-unique(Aspen_0.1P$Event_ID)

ALL_LC_df <- ALL_LC_df[ALL_LC_df$Event_ID %in% Fire_Selection, ]

#Create aspen only df: 
Aspen_DOB<-ALL_LC_df[ALL_LC_df$Group_Class=="aspen", ]#Now has the correct 1687 DOBs
...
#Additive term model:
modelA<-glmmTMB(log10(DOB_Max_DS)~GC_Percent +  bui_raw + dsr_raw + dc_raw	+ dmc_raw	+ ffm_raw	+ fwi_raw	+ isi_raw	+ tmax_raw + prec_raw	+ ws_raw + rh_raw	 + Slp_raw	+ Rugg_raw + Elev_raw	+ Northness	+ Eastness	+ M_cwd_raw	+ M_tmax_raw + sin.doy + (1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(modelA)

#All significant additive that aren't correlated above 0.3 r2:
#Additive term model:
modelAS<-glmmTMB(log10(DOB_Max_DS)~GC_Percent + fwi_raw + sin.doy + (1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(modelAS)
#Find r^2
...
#Additive term model: FOR TESTING
modelA<-glmmTMB(log10(DOB_Max_DS)~GC_Percent + fwi_raw + (1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(modelA)

#Two dimension model: FOR TESTING
modelI<-glmmTMB(log10(DOB_Max_DS)~GC_Percent * M_cwd_raw +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(modelI)

#Now create model selection table for each spread variable with all covariates as additive terms and selected interaction terms

#+bui_raw
model2<-glmmTMB(log10(DOB_Max_DS)~GC_Percent + bui_raw +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(model2)

#+dsr_raw
model3<-glmmTMB(log10(DOB_Max_DS)~GC_Percent + dsr_raw +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(model3)

#+dc_raw
model4<-glmmTMB(log10(DOB_Max_DS)~GC_Percent + dc_raw +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(model4)

#+dmc_raw
model5<-glmmTMB(log10(DOB_Max_DS)~GC_Percent + dmc_raw +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(model5)

#+ffm_raw
model6<-glmmTMB(log10(DOB_Max_DS)~GC_Percent + ffm_raw +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(model6)

#+fwi_raw
model7<-glmmTMB(log10(DOB_Max_DS)~GC_Percent + fwi_raw +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(model7)

#+isi_raw
model8<-glmmTMB(log10(DOB_Max_DS)~GC_Percent + isi_raw +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(model8)

#+tmax_raw
model9<-glmmTMB(log10(DOB_Max_DS)~GC_Percent + tmax_raw +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(model9)

#+prec_raw
model10<-glmmTMB(log10(DOB_Max_DS)~GC_Percent + prec_raw +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(model10)

#+ws_raw
model11<-glmmTMB(log10(DOB_Max_DS)~GC_Percent + ws_raw +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(model11)

#+rh_raw
model12<-glmmTMB(log10(DOB_Max_DS)~GC_Percent + rh_raw +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(model12)

#+Northness
model13<-glmmTMB(log10(DOB_Max_DS)~GC_Percent + Northness +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(model13)

#+Elev_raw
model14<-glmmTMB(log10(DOB_Max_DS)~GC_Percent + Elev_raw +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(model14)

#+Slp_raw
model15<-glmmTMB(log10(DOB_Max_DS)~GC_Percent + Slp_raw +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(model15)

#+Rugg_raw
model16<-glmmTMB(log10(DOB_Max_DS)~GC_Percent + Rugg_raw +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(model16)

#+Eastness
model17<-glmmTMB(log10(DOB_Max_DS)~GC_Percent + Eastness +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(model17)

#+I(sin.doy)
model18<-glmmTMB(log10(DOB_Max_DS)~GC_Percent + I(sin.doy) +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(model18)

#Monthly: 
#+M_cwd_raw
model19<-glmmTMB(log10(DOB_Max_DS)~GC_Percent + M_cwd_raw +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(model19)

#+M_tmax_raw
model20<-glmmTMB(log10(DOB_Max_DS)~GC_Percent + M_tmax_raw +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(model20)

#+M_vpd_raw
model21<-glmmTMB(log10(DOB_Max_DS)~GC_Percent + M_vpd_raw +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(model21)

#Try some interactive effects: 
#GC_Percent * fwi_raw
model22<-glmmTMB(log10(DOB_Max_DS)~GC_Percent * fwi_raw +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(model22)

#GC_Percent * dsr_raw
model23<-glmmTMB(log10(DOB_Max_DS)~GC_Percent * dsr_raw +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(model23)

#GC_Percent * rh_raw
model24<-glmmTMB(log10(DOB_Max_DS)~GC_Percent * rh_raw +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(model24)

#GC_Percent * tmax_raw
model25<-glmmTMB(log10(DOB_Max_DS)~GC_Percent * tmax_raw +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(model25)

#GC_Percent * I(sin.doy)
model26<-glmmTMB(log10(DOB_Max_DS)~GC_Percent * I(sin.doy) +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(model26)

#GC_Percent * ffm_raw
model27<-glmmTMB(log10(DOB_Max_DS)~GC_Percent * ffm_raw +(1|Event_ID), family=gaussian, data=Aspen_DOB)
summary(model27)


#Make model selection table:
model_list <- list(model1, model2, model3, model4, model5, model6, model7, model8, model9, model10, model11, model12, model13, model14, model15, model16, model17, model18, model19, model20, model21, model22, model23, model24, model25, model26, model27)  

# Extract AIC values for each model
aic_values <- sapply(model_list, AIC)

# Create a data frame with model names and corresponding AIC values
model_names <- paste0("Model", 1:length(model_list))
model_selection_table <- data.frame(Model = model_names, AIC = aic_values)

# Sort the table by AIC values in ascending order
model_selection_table <- model_selection_table[order(model_selection_table$AIC), ]


#Now use the model selection table to make Table S4 depicting %aspen model and top five best fit models for each spread varaible:

#Find r^2
R2 <- r.squaredGLMM(model22)
R2
#Back-calculate intercept: 
10^2.935
